...
class TestSerialize {
    ...
    var strOpt: String?
    var intOpt: Int?
    var objOpt: TestInternalSerialize?
    var arrayOpt: [TestInternalSerialize] = []
...
test.strOpt = "abcd"
test.objOpt = TestInternalSerialize()
test.objOpt?.inIntOpt = 1234

println(NSJSONSerialization.isValidJSONObject(test))

if let data = NSJSONSerialization.dataWithJSONObject(test, options: nil, error: nil) {
    println(data)
